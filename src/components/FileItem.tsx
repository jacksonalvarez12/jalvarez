...
export default function FileItem({ item, onNavigate, onDelete }: Props) {
          ...
          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
            {confirmDelete ? (
              <>
                ...
                <button
                  onClick={() => { onDelete(item.fullPath); setConfirmDelete(false) }}
  ...
  return (
    <tr className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors group">
      ...
      <td className="py-2.5 px-4 text-gray-400 text-sm">{formatBytes(item.size)}</td>
      <td className="py-2.5 px-4 text-gray-400 text-sm">{formatDate(item.updatedAt)}</td>
      <td className="py-2.5 px-4">
