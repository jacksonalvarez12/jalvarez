...
export function useStorage() {
      ...
      files.forEach((file, i) => {
        ...
        task.on(
          ...
          (error) => {
            setUploads((prev) =>
              prev.map((u, idx) =>
                idx === globalIndex ? { ...u, done: true, error: error.message } : u
              )
            )
          },
          () => {
  ...
  const createFolder = useCallback(async (currentPath: string, folderName: string) => {
    const keepRef = ref(
      storage,
  ...
  return { items, loading, uploads, listPath, uploadFiles, createFolder, deleteItem, clearUploads }
}
